...
# -*- coding: utf-8 -*-
"""
Миграция: Добавление реестра брендов (Brand Registry)

Создаёт таблицы:
- brands — централизованный реестр брендов
- brand_aliases — маппинг вариантов написания
- brand_category_links — допустимость бренда в категориях WB
...
def run_migration():
    db_path = get_db_path()
    logger.info(f"Миграция Brand Registry: {db_path}")
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_brand_wb_id ON brands(wb_brand_id)')
        ...
        # ============================================================
        # 3. Создание таблицы brand_category_links
        # ============================================================
        ...
        if not cursor.fetchone():
            ...
            cursor.execute('''
                CREATE TABLE brand_category_links (
                    ...
                    verified_at TIMESTAMP,
                    CONSTRAINT uq_brand_category UNIQUE (brand_id, wb_subject_id)
                )
            ''')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_bcl_brand ON brand_category_links(brand_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_bcl_subject ON brand_category_links(wb_subject_id)')
            logger.info("Таблица brand_category_links создана")
        else:
            logger.info("Таблица brand_category_links уже существует")

        # ============================================================
        # 4. Добавление resolved_brand_id в imported_products
        # ============================================================
        ...
        if 'resolved_brand_id' not in columns:
            ...
            logger.info("resolved_brand_id добавлен в imported_products")
        ...
        if 'brand_status' not in columns:
            ...
            logger.info("brand_status добавлен в imported_products")

        # ============================================================
        # 5. Добавление resolved_brand_id в supplier_products
        # ============================================================
        ...
        if 'resolved_brand_id' not in columns:
            ...
            logger.info("resolved_brand_id добавлен в supplier_products")

        # ============================================================
        # 6. Seed данных из BRAND_CANONICAL
        # ============================================================
        ...
        if brand_count == 0:
            ...
            for alias_lower, canonical_name in BRAND_CANONICAL.items():
                ...
                # Проверяем что alias ещё не существует
                ...
                # Также добавляем каноническое имя как alias (для точного матча)
            ...
            # ============================================================
            # 7. Извлечение уникальных брендов из supplier_products
            # ============================================================
        ...
        conn.commit()
        logger.info("Миграция Brand Registry завершена успешно!")
