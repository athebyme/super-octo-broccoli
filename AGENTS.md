...
в настроенную социальную сеть и не изменяет marketplace listing.
...
- Docker healthcheck обязан задавать внутренний network timeout короче container timeout и закрывать ответ. Web `start-period=180s` учитывает fail-fast startup migrations на многогигабайтной SQLite БД; не сокращайте его ниже измеренного migration startup без отдельной проверки. Agent liveness обновляется локальным потоком без I/O и не зависит от доступности platform heartbeat/poll.
