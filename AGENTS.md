...
- `scripts/probe_ozon_read_contracts.py`: optional live read-only contract probe для catalog/warehouses и finance accrual types/current-day. Он принимает credentials только из process env или owner-only `/tmp/ozon_live.env`, вызывает исключительно manifest endpoints с `retry_class=read` и выводит только bounded response shapes без scalar values/offer/SKU/raw errors.
...
- Актуальный Ozon manifest использует description-category v1, product list/info v3, product attributes v4, pictures read v2/write v1, product import v3 + status v1, archive/unarchive v1, limits v4, prices read v5/update v1, aggregate stocks read v4, per-warehouse FBS read v2, per-warehouse FBO read v1, stocks update v2, warehouses v2, analytics data v1, postings FBS v4/FBO v3, returns v1/rFBS v2, conditional cancellation v2 и finance accrual by-day/types/postings v1. Deprecated category/product endpoints, postings FBS v3/FBO v2, conditional cancellation v1, per-warehouse FBS v1, warehouse v1 и устаревающие finance transaction v3 запрещены. По уведомлению Ozon от 14.07.2026 finance v3 отключат 08.09.2026; не добавляйте временный fallback. В `/v3/product/import` `offer_id` обязателен, а `images360` удалён 10.07.2026.
...
- Ozon analytics sync/fact/quality, fulfillment posting/item/status/return/cancellation и finance snapshot/fact/component rows всегда выбираются по полному `seller_id + marketplace_id + account_id`; bare sync/posting/listing/account/fact ID недостаточен. `account_id` для insight/fulfillment/finance routes задаётся только canonical positive query-параметром, а body не может подменить marketplace scope.
