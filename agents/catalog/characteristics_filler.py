...
КРИТИЧЕСКИЕ ПРАВИЛА:
- ЗАПРЕЩЕНО выдумывать характеристики! Сначала ОБЯЗАТЕЛЬНО вызови get_category_characteristics(subject_id=...)
  чтобы узнать реальный набор характеристик для категории товара
- Для справочных значений (цвет, страна, пол, сезон) используй get_directory(directory_type=...)
  чтобы взять значение из реального справочника WB
...
2. Получить характеристики категории (get_category_characteristics) — ОБЯЗАТЕЛЬНО
3. Для цветов/стран/пола/сезона — получить справочник (get_directory)
4. Извлечь значения из описания товара, сопоставить с допустимыми значениями из словарей
5. Заполнить характеристики — использовать ТОЛЬКО значения из справочников
6. Сохранить (update_imported_product)
        ...
        if task_type == 'fill_single':
            ...
            if imported_product_id:
                return (
                    ...
                    f"2. get_category_characteristics(subject_id=<wb_subject_id из товара>) — ОБЯЗАТЕЛЬНО узнай реальные характеристики категории\n"
                    f"3. Для цвета: get_directory(directory_type='colors', query=<цвет из описания>)\n"
                    f"4. Для страны: get_directory(directory_type='countries', query=<страна из описания>)\n"
                    f"5. Извлеки данные из описания, сопоставь с допустимыми значениями из словарей\n"
                    f"6. update_imported_product(product_id={imported_product_id}, characteristics=<JSON характеристик>)\n\n"
