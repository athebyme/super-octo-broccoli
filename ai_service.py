...
ВАЖНО: Отвечай ТОЛЬКО валидным JSON."""
    ...
    "rich_content": {
        ...
        "template": """Ты топовый копирайтер для Wildberries, специализирующийся на создании Rich-контента.

Rich-контент на WB - это блоки с изображениями и текстовыми наложениями (до 10 блоков).
Каждый блок = 1 изображение 1440x810px + текст поверх.

ТРЕБОВАНИЯ WB К RICH-КОНТЕНТУ:
1. ЗАПРЕЩЕНО: эмодзи, водяные знаки, цены, скидки, QR-коды, ссылки
2. ЗАПРЕЩЕНО: "хит продаж", "лучшая цена", призывы позвонить
3. Текст должен быть коротким - для наложения на изображение
4. Максимум 10 блоков

СТРУКТУРА БЛОКОВ:
1. Главный блок - крупный заголовок + УТП товара
2. Проблема/решение - какую боль закрывает товар
3-4. Ключевые преимущества (по одному на блок)
5-6. Характеристики/материалы
7-8. Применение/сценарии использования
9. Комплектация или особенности
10. Гарантия/доверие

ПРАВИЛА ТЕКСТА:
- Заголовок блока: 3-7 слов, крупно
- Подзаголовок: 1-2 коротких предложения
- БЕЗ эмодзи (запрещено на WB!)
    ...
    "blocks": [
        {
            ...
            "subtitle": "Короткое пояснение в 1-2 предложения",
            "image_idea": "Идея для фото: что должно быть изображено"
        },
        {
            "number": 2,
            "type": "problem",
            "title": "Заголовок про проблему",
            "subtitle": "Как товар решает эту проблему",
            "image_idea": "Идея для фото"
        }
    ],
    "total_blocks": <число блоков 5-10>,
    "main_message": "Главный посыл карточки в одном предложении",
    "target_audience": "Для кого этот товар"
}

ВАЖНО: Создай 5-10 блоков для инфографики. БЕЗ ЭМОДЗИ! Отвечай ТОЛЬКО валидным JSON."""
    },
